...
class Users {
    ...
    public function register($data = array()) {
        ...
        $user['instagrem']            = (ISSET($data['instagrem']) ? Secure($data['instagrem']) : '');
    ...
    public function get_user_profile($username, $cols = array(),$only_token = false) {
        ...
        if ($db->count > 0) {
            if( $columns[0] == "*" ) {
                ...
                $mediafiles = $db->where('user_id', trim($profile->id))->orderBy('id', 'desc')->get('mediafiles', null, array('id','file'));
                if ($mediafiles) {
                    $mediafilesid = 0;
                    foreach ($mediafiles as $mediafile) {
                        $mf = array(
                ...
                foreach ($result as $key => $value) {
                    ...
                    if( isset( $result->id ) ) {
                        $profile->mediafiles = array();
                        $mediafiles = $db->where('user_id', trim($result->id))->orderBy('id', 'desc')->get('mediafiles', null, array('id','file'));
                        if ($mediafiles) {
                            $mediafilesid = 0;
                            foreach ($mediafiles as $mediafile) {
    ...
    public function matches(){
        ...
        if (empty($_POST['access_token'])) {
            ...
            $exclude_text = $_userid . ',' . implode(',',$exclude);
    ...
    public function update_avater(){
        ...
        if (empty($_FILES['avater']) || empty($_POST['access_token'])) {
            ...
            if (move_uploaded_file($_FILES['avater']['tmp_name'], $filename)) {
                $thumbfile = $dir . $_DS . $key . '_avater.' . $ext;

    ...
    <div class="row r_margin">
        <div class="col s12 m3 custom_fixed_element">
            ...
            <div class="dt_user_profile">
                ...
                <?php if( isset( $_COOKIE[ 'JWT' ] ) && !empty( $_COOKIE[ 'JWT' ] ) ){ ?>
					<div class="dt_user_pro_info">
						<ul class="dt_ldl_prof">
							...
							<li class="dislike">
								<a href="javascript:void(0);" id="dislike_btn" data-replace-text="<?php echo __('Disliked');?>" data-replace-dom=".dislike_text" data-ajax-post="/useractions/<?php if( isUserDisliked( $profile->id ) ) { echo 'remove_dislike'; } else { echo 'dislike'; }?>" data-ajax-params="userid=<?php echo $profile->id;?>&web_device_id=<?php echo $profile->web_device_id;?>" data-ajax-callback="callback_<?php if( isUserDisliked( $profile->id ) ) { echo 'remove_dislike'; } else { echo 'dislike'; }?>">
                ...
                <?php if( !empty( $profile->facebook ) || !empty( $profile->twitter ) || !empty( $profile->google ) || !empty( $profile->instagrem ) || !empty( $profile->linkedin ) || !empty( $profile->website ) ) {?>
                    ...
                    <ul>
                        ...
                        <?php } ?>
                        <?php if( !empty( $profile->instagrem ) ) {?>
                        <li class="insta">
                            <a href="https://www.instagram.com/<?php echo $profile->instagrem;?>" target="_blank">
                                <div class="soc_icon">
                                ...
                                <div class="soc_info">
                                    ...
                                    <span>@<?php echo $profile->instagrem;?></span>
                                </div>
                            </a>
                        </li>
                        <?php } ?>
						...
						<?php
							...
							foreach ($gallery['visable'] as $key => $value) {
								if( !empty( $value ) ){
									...
									echo '<a class="inline" href="'. $value['full'] .'" data-fancybox="gallery"><img src="' . $value['avater'] .'" alt="' . $profile->username .'"></a>';
									echo '</div>';
								}else{
									echo '<div class="dt_cp_l_photos">';

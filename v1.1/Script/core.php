...
function CompressImage($source_url, $destination_url, $quality) {
    ...
    @$image_c($image, $destination_url, $quality);
...
function ShareFile($data = array(), $type = 0, $crop = true,$fldr=false) {
    ...
    if( $folder == 'gifts' || $folder == 'stickers' ){
        $filename    = $_UPLOAD . "{$folder}" . DIRECTORY_SEPARATOR . $fn;
    }
...
function GetFindMatcheQuery($user_id, $limit, $offset){
    $where_or = array();
    $where_and = array();
    $distance = '';
    ...
    $u = auth();
    if( $u->show_me_to !== '' ) {
        $query = 'SELECT DISTINCT * FROM `users` WHERE `country` = "' . $u->show_me_to .'"';

        $blocked_user_array = (array_keys(BlokedUsers($u->id))) ? array_keys(BlokedUsers($u->id)) : array();
        if (is_array($blocked_user_array)) {
            if (count($blocked_user_array) > 0) {
                $query .= ' AND `id` NOT IN (' . implode($blocked_user_array, ',') . ') ';
            }
        }
        $liked_user_array = (array_keys(LikedUsers($u->id))) ? array_keys(LikedUsers($u->id)) : array();
        if (is_array($liked_user_array)) {
            if (count($liked_user_array) > 0) {
                $query .= ' AND `id` NOT IN (' . implode($liked_user_array, ',') . ') ';
            }
        }
        $disliked_user_array = (array_keys(DisLikedUsers($u->id))) ? array_keys(DisLikedUsers($u->id)) : array();
        ...
        $orderBy .= ',`is_pro` DESC';
        $orderBy .= ',`id` DESC';
        $query = $query . $orderBy . ' LIMIT '.$limit.' OFFSET '.$offset.';';
        return $query;
//        $where_or[] = '`show_me_to` = "' . $u->show_me_to .'"';
//        $where_or[] = '`country` = "' . $u->show_me_to .'"';
    }
    if( $u->country !== '' ) {
        $where_or[] = '`show_me_to` = "' . $u->country .'"';
        $where_or[] = '`country` = "' . $u->country .'"';
    }

    if( $u->show_me_to == '' ) {
        if (
            ( isset($_POST['_lat']) && !empty($_POST['_lat']) && isset($_POST['_lng']) && !empty($_POST['_lng']) && !empty($_POST['_located']) )
            ||
            ( isset($_SESSION['_lat']) && !empty($_SESSION['_lat']) && isset($_SESSION['_lng']) && !empty($_SESSION['_lng']) && !empty($_SESSION['_located']) )
        ) {
            $lat = 0;
            $lng = 0;
            $located = 7;
    ...
    if (isset($_POST) && !empty($_POST)) {
        ...
        if( isset($_POST['_location']) && !empty($_POST['_location']) ){
            $where_or[] = '`location` like "%'. Secure($_POST['_location']) .'%"';
        }
        ...
        if( isset($_POST['_gender']) && $_POST['_gender'] !== ''){
            if( strpos( Secure( $_POST['_gender'] ), ',' ) === false ) {
                $where_and[] = '`gender` = "'. Secure($_POST['_gender']) .'"';
            }else{
                $where_and[] = '`gender` IN ('. Secure($_POST['_gender']) .')';
            }
        }else{
        ...
        if( !empty($where_or) ){
            $where = $where . '('. implode($where_or, ' OR ') . ') ';
        }
        if( !empty($where_and) ){
            if( !empty($where_or) ) {
                $where = $where . ' OR (' . implode($where_and, ' AND ') . ')';
            }else{
        ...
        if(is_array($blocked_user_array)) {
            if (count($blocked_user_array) > 0) {
                $notin .= ' AND `id` NOT IN (' . implode($blocked_user_array, ',') . ')';
            }
        }
        $liked_user_array = ( array_keys(LikedUsers($user_id)) ) ? array_keys(LikedUsers($user_id)) : array();
        if(is_array($liked_user_array)) {
            if (count($liked_user_array) > 0) {
                $notin .= ' AND `id` NOT IN (' . implode($liked_user_array, ',') . ')';
            }
        }
        $disliked_user_array = ( array_keys(DisLikedUsers($user_id)) ) ? array_keys(DisLikedUsers($user_id)) : array();
        if(is_array($disliked_user_array)) {
            if (count($disliked_user_array) > 0) {
                $notin .= ' AND `id` NOT IN (' . implode($disliked_user_array, ',') . ')';
            }
        }
    }
    if( $limit == 0 ){
    ...
    $orderBy .= ',`is_pro` DESC';
    $query = $query . $where . ' ) ' . $notin . $orderBy . ' LIMIT '.$limit.' OFFSET '.$offset.';';
...
function CreateNewAudioCall($re_data) {
    global  $conn;
    if (IS_LOGGED == false) {
        return false;
    }
...
function CreateNewVideoCall($re_data) {
    global $conn;
    if (IS_LOGGED == false) {
        return false;
    }
...
function CheckCallAnswer($id = 0) {
    ...
    if (mysqli_num_rows($query) > 0) {
        $sql          = mysqli_fetch_assoc($query);
        $data1['url'] = $config->uri . '/video-call/' . $id;
        return $data1;
    } else {
        return false;
    }
}
function CheckCallAnswerDeclined($id = 0) {
    global $conn,$config;
    if (IS_LOGGED == false) {
        return false;
    }

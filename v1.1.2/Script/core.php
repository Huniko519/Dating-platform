...
function ProUsers(){
    ...
    $pro_users = $db->objectBuilder()
        ->where( 'verified', '1' )
        //->where( 'id', auth()->id, '<>' )
        ->where("(is_pro = 1 OR is_boosted = 1)")
        ->where("id NOT IN (SELECT like_userid FROM likes WHERE is_dislike = 1 AND user_id = '$u->id')")
        ->where("id NOT IN (SELECT block_userid FROM blocks WHERE user_id = '$u->id')")
...
function GetFindMatcheQuery($user_id, $limit, $offset){
    ...
    if (is_array($blocked_user_array)) {
        if (count($blocked_user_array) > 0) {
            foreach ($blocked_user_array as $key => $value){
                $excludes[$key] = $value;
            }
        }
    }
    ...
    if (is_array($liked_user_array)) {
        if (count($liked_user_array) > 0) {
            foreach ($liked_user_array as $key => $value){
                $excludes[$key] = $value;
            }
        }
    }
    ...
    if (is_array($disliked_user_array)) {
        if (count($disliked_user_array) > 0) {
            foreach ($disliked_user_array as $key => $value){
                $excludes[$key] = $value;
            }
        }
    }
    ...
    $notin = ' AND `id` NOT IN (' . implode($excludes, ',') . ') AND `id` <> "' . $u->id .'" ';

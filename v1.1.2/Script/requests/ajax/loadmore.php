...
Class Loadmore extends Aj {
    function random_users() {
        ...
        if ($error == '') {
            ...
            $exclude = (array(array_merge($blocked_user_array, $liked_user_array, $disliked_user_array)));
            $sql = 'SELECT `id`,`online`,`lastseen`,`username`,`avater`,`country`,`first_name`,`last_name`,`location`,`birthday`,`language`,`relationship`,`height`,`body`,`smoke`,`ethnicity`,`pets` FROM `users` ';
            $sql .= ' WHERE ( ';
            $sql .= ' `active` = "1" AND `privacy_show_profile_random_users` = "1" ';
            if (!empty($exclude[0])) {
                    $sql .= ' AND `id` NOT IN (' . implode(',',$exclude[0]) . ') ';
            }
            $sql .= ' ) ';

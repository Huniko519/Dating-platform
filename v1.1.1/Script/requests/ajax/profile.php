...
Class Profile extends Aj {
    ...
    public function save_social_setting() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if (isset($_POST[ 'instagrem' ])) {
                if (filter_var($_POST[ 'instagrem' ], FILTER_VALIDATE_URL)) {
                    $error .= '<p>• ' . __('Please enter just instagrem profile user.') . '</p>';
                } else {
                    $user[ 'instagrem' ] = Secure($_POST[ 'instagrem' ]);
                }
            }
    ...
    public function upload_receipt() {
        ...
        foreach ($_FILES as $file) {
            ...
            if (move_uploaded_file($file[ 'tmp_name' ], $filename)) {
                ...
                $info[ 'receipt_file' ]= 'upload/photos/' . date('Y') . '/' . date('m') . '/' . $key . '_full.' . $ext;
